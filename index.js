...
class ServerCloner {
    ...
    async cloneServer(sourceGuildId, targetGuildId, cloneEmojis = true, progressChannel = null) {
        try {
            ...
            this.sendProgress('Codex Starting cloning process...', progressChannel);
    ...
    if (pendingOperations.has(message.author.id)) {
        ...
        if (operation.step === 'confirmProceed') {
            ...
            if (messageContent === 'y' || messageContent === 'yes') {
                ...
                pendingOperations.delete(message.author.id);
                message.channel.send('🚀 Codex Starting server cloning process...').then(sentMsg => {
                    botMessageIds.add(sentMsg.id);
                }).catch(() => {});
                try {
                ...
                pendingOperations.delete(message.author.id);
                message.channel.send('🚀 Codex Starting server cloning process (without emojis)...').then(sentMsg => {
                    botMessageIds.add(sentMsg.id);
                }).catch(() => {});
                try {
                    const cloner = new ServerCloner(client);
...
client.login(process.env.TOKEN).then(() => {
    log.success('Codex-log:Logged in successfully!');
}).catch((error) => {
    log.error(`Codex-log:Login failed: ${error.message}`);
    process.exit(1);
});
